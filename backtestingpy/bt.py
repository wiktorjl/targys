...
class BearishToBullish(SignalStrategy, TrailingStrategy):
    n1 = 5
    n2 = 30
    ...
    def init(self):
        ...
        # ma1 is a an SMA. Calculate when its sloper turns positive.
        self.ma1_positive = self.ma1 > self.ma1[-1] and self.ma1[-1] < self.ma1[-2]

        # Set to 0 if negative or Nan, 1 if positive
        # self.ma1_positive = self.ma1_positive.fillna(0).astype(int)

        

        self.set_signal(entry_size=self.ma1_positive)
...
def task(ticker, data):
    ...
    bt = Backtest(data, BearishToBullish, commission=.002, exclusive_orders=True)
    stats = bt.run()
